...
def test_adsb_to_cot_xml(sample_feed):
    ...
    cot = kraktak.functions.adsb_to_cot_xml(sample_craft)
...
def test_adsb_to_cot(sample_feed):
    ...
    sample_craft = sample_feed["aircraft"][0]
    cot = kraktak.adsb_to_cot(sample_craft)
    expected = (
        ...
        b'Squawk: None Category: A1 (via kraktak@rorqual)"><UID Droid="ICAO-A9EE47" />'
        b'<contact callsign="N739UL--" /><track course="50.1" speed="40.641076" /><remarks>N739UL-- ICAO: '
        b"A9EE47 REG:  Flight: N739UL Type:  Squawk: None Category: A1 (via kraktak@rorqual)</remarks>"
        b"</detail></event>"
    )
...
def test_adsb_to_cot_with_known_craft(sample_feed, sample_known_craft):
    ...
    cot = kraktak.functions.adsb_to_cot_xml(sample_craft, known_craft=known_craft)
    print(ET.tostring(cot))
